...
La plupart des données sont **déjà** dans la base SQLAlchemy (`jdr_database.sqlite`). Seuls `chat_sessions` / `chat_messages` vivent dans `alezia.db` (brut). La migration se réduit donc à déplacer ces deux tables.
...
2. **Base de données unique (le gros morceau).**
   1. Créer les modèles ORM `ChatSession` / `ChatMessage` (`backend/models/chat.py`), reliés à `CharacterModel`.
   2. Réécrire `chat_service` pour utiliser `SessionLocal` / l'ORM à la place de `db_manager` (méthodes : `create_session`, `get_session`, `get_user_sessions`, `delete_session`, `get_session_messages`, `send_message`, `add_message`, `generate_response`).
   3. **Script de migration one-shot** (`scripts/migrate_to_single_db.py`) : sauvegarder les deux `.db`, puis lire `chat_sessions` / `chat_messages` depuis l'ancien `alezia.db` brut et les réinsérer comme lignes ORM dans la base SQLAlchemy (`jdr_database.sqlite`).
   4. **Ordre anti-collision de nommage** : migration TERMINÉE → supprimer l'ancien `alezia.db` brut → renommer `jdr_database.sqlite` en `data/alezia.db` → pointer `DB_PATH` sur ce fichier unique. Retirer `db_manager` (`backend/utils/db.py`), `backend/utils/schema.sql`, et la référence dans `routes/system.py`.
   5. Adapter `init_db.py` (qui utilisait `db_manager.ensure_default_universe`) pour initialiser via l'ORM (`Base.metadata.create_all` + univers par défaut).
   - *Vérif :* anciens personnages **et** anciennes conversations présents ; chat répond ; un seul `.db` reste.
...
- Données historiques (personnages + conversations) préservées dans la base unique.
- `pytest` passe ; `ruff check` propre.
- `CLAUDE.md` décrit fidèlement le projet aligné.

## Risques

- **Réécriture de `chat_service` sur l'ORM** : principal point de complexité. Mitigation : migration de données vérifiée avant suppression de `db_manager`, et conservation d'une copie de sauvegarde des `.db` avant migration.
- **Différences de schéma** entre `schema.sql` (brut) et les modèles ORM pour sessions/messages : à réconcilier explicitement lors de la création des modèles `ChatSession`/`ChatMessage`.
