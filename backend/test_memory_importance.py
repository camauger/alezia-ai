...
def get_or_create_test_character():
    ...
    if characters:
        ...
        print(f'Utilisation du personnage de test existant (ID: {character_id})')
    else:
...
def test_importance_calculation(character_id):
    ...
    for i, memory_data in enumerate(test_memories):
        ...
        calculated_importance = memory_manager._calculate_memory_importance(
            memory_data['content'], memory_data['type']
        )
        ...
        print(
            f'Contenu: {memory_data["content"][:50]}...'
            if len(memory_data['content']) > 50
            else f'Contenu: {memory_data["content"]}'
        )
        print(
...
def test_importance_reevaluation(character_id):
    ...
    for i in range(10):
        ...
        if i in [0, 4, 9]:
            ...
            print(
                f'Après {i + 1} accès - Importance: {current_memory["importance"]:.2f}, '
...
def test_memory_decay(character_id):
    ...
    update_count = memory_manager.decay_old_memories(character_id, days_threshold=30)
    ...
    if final_importances['old_low'] < initial_importances['old_low']:
        ...
        print(
            "❌ La mémoire ancienne de faible importance n'a pas été dégradée (incorrect)"
        )

    if final_importances['old_high'] <= initial_importances['old_high']:
        if final_importances['old_high'] == initial_importances['old_high']:
            print(
                "✅ La mémoire ancienne de haute importance n'a pas été dégradée en raison de son importance élevée"
            )
        else:
            print(
                '✅ La mémoire ancienne de haute importance a été légèrement dégradée'
            )
    else:
...
def test_memory_consolidation(character_id):
    ...
    similar_content1 = (
        "J'aime vraiment beaucoup la musique classique, surtout les œuvres de Mozart."
    )
    ...
    memories_after = db_manager.execute_query(
        'SELECT id, importance FROM memories WHERE id IN (?, ?, ?)', (id1, id2, id3)
    )
