...
def display_traits(traits):
    ...
    for trait in traits.traits:
        ...
        if trait.value > 0:
            bar_str = (
                ''.join(bar[:middle]) + '\033[32m' + ''.join(bar[middle:]) + '\033[0m'
            )
        else:
            bar_str = (
                '\033[31m' + ''.join(bar[:middle]) + '\033[0m' + ''.join(bar[middle:])
            )

        print(f'{trait.name:20} [{bar_str}] {trait.value:+.2f} ({trait.category})')
...
def test_trait_evolution(character_id, character_manager, messages):
    ...
    for initial_trait in initial_traits.traits:
        ...
        if final_trait:
            ...
            print(
                f'{initial_trait.name}: {initial_trait.value:.2f} → {final_trait.value:.2f} {direction} ({total_change:+.2f})'
            )
...
def main():
    ...
    for entry in history:
        change_symbol = (
            ...
            else ('↓' if entry.change_amount < 0 else '→')
        )
        print(
            f'{entry.timestamp.strftime("%H:%M:%S")} - {entry.trait_name}: {entry.old_value:.2f} → {entry.new_value:.2f} {change_symbol} ({entry.change_amount:+.2f})'
        )
