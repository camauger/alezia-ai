...
class PersonalityService:
    ...
    def initialize_personality_traits(self, db: Session, character_id: int, initial_traits: List[Dict[str, Any]]) -> None:
    ...
    def get_personality_traits(self, db: Session, character_id: int) -> PersonalityTraits:
        ...
        traits_data = db.query(TraitModel).filter(TraitModel.character_id == character_id).all()
        return PersonalityTraits(traits=traits_data)

    def get_personality_traits_as_dict(self, db: Session, character_id: int) -> Dict[str, float]:
        """Retrieves the personality traits of a character as a dictionary {name: value}"""
        traits_data = db.query(TraitModel.name, TraitModel.value).filter(TraitModel.character_id == character_id).all()
    ...
    def get_trait_history(self, db: Session, character_id: int, trait_name: str = None) -> List[TraitChange]:
        ...
        query = db.query(TraitChangeModel).filter(TraitChangeModel.character_id == character_id)
        if trait_name:
            query = query.join(TraitModel).filter(TraitModel.name == trait_name)
        return query.order_by(TraitChangeModel.timestamp.desc()).all()

    def update_trait(self, db: Session, character_id: int, trait_name: str, new_value: float, reason: str) -> Optional[TraitModel]:
        """Updates the value of a personality trait and records the change"""
        db_trait = db.query(TraitModel).filter(
            TraitModel.character_id == character_id,
