...
async def get_character(character_id: int = Path(..., ge=1), db: Session = Depends(get_db)):
...
async def delete_character(character_id: int = Path(..., ge=1), db: Session = Depends(get_db)):
    """Deletes a character"""
    try:
...
async def get_character_state(character_id: int = Path(..., ge=1), db: Session = Depends(get_db)):
...
async def get_character_traits(character_id: int = Path(..., ge=1), db: Session = Depends(get_db)):
...
async def get_trait_history(
    ...
    trait_name: Optional[str] = Query(None, description="Name of the specific trait to consult"),
