...
class UniverseBase(BaseModel):
    ...
    type: str = Field(
        ..., description='Type of universe (fantasy, sci-fi, historical, etc.)'
    )
...
class UniverseElementBase(BaseModel):
    ...
    universe_id: int = Field(
        ..., description='ID of the universe this element belongs to'
    )
    name: str = Field(..., description='Name of the element')
    type: str = Field(
        ..., description='Type of element (location, organization, item, concept, etc.)'
    )
    description: str = Field(..., description='Detailed description of the element')
    importance: int = Field(
        3, description='Importance of the element in the universe (1-5)'
    )
